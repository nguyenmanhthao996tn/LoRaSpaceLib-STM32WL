...
*
* Arguments   : *payload pointer to the buffer cointaining the data to de/encrypt
*				payload_len len of data to de/encrypt
*				*Key pointer to application key
*				*DevAddr pointer to device address
*				Direction uplink (0x00) or downlink (0x01)
*				Frame_Counter frame counter of packet
*				*Final_MIC
*****************************************************************************************
*/
							...
							unsigned char *DevAddr, uint8_t Direction, unsigned int Frame_Counter, unsigned char *MIC)
{
    ...
    //Add data to it
    for(i = 0x00; i < payload_len; i++)
    {
        MIC_Data[i + 16] = payload[i];
    }

    //Calculate the correct buffer length
    MIC_len = 16 + payload_len;

    //Calculate the MIC
    Calculate_MIC(MIC_Data, MIC_len, NwkSKey, MIC);
}
					...
					unsigned char *MIC)
{
	...
	if(Incomplete_Block_Size == 0)
	{
		...
		for(i = 0; i < 16; i++)
		{
			New_Data[i] = payload[((Number_of_Blocks -1)*16)+i];
		}
